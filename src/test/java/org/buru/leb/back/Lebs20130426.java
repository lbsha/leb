...
import org.buru.sardine.Functions.ICndAction;
import org.buru.sardine.Functions.ICndActionR;
...
public final class Lebs20130426 {
	...
	private void prepareBucket(final Object observer) {
		Functions.foreach(Containers.newArrayList(observer.getClass().getMethods()), new ICndAction<Method>() {
			Observe observe;
	...
	public void noticeRegex(final String hintExpression, final Object... arguments) {
		...
		Functions.foreach(keys, new ICndAction<String>() {
	...
	private <T> Collection<Callable<T>> toCallable(final Collection<MethodWrapper> wrappers, final Object... arguments) {
		return Functions.foreach(wrappers, new ICndActionR<MethodWrapper, Callable<T>>() {
			// 过滤掉一部分参数不匹配的
